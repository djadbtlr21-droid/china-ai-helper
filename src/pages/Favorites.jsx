...
function AddPlaceModal({ onClose, onSave }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:400, background:'rgba(26,16,8,0.92)', backdropFilter:'blur(8px)', display:'flex', alignItems:'flex-end' }} onClick={onClose}>
      <div style={{ width:'100%', background:'var(--ink)', borderRadius:'24px 24px 0 0', padding:'20px 20px 48px', maxHeight:'90vh', overflowY:'auto', border:'1px solid var(--card-border)', borderBottom:'none' }} onClick={e => e.stopPropagation()}>
        <div style={{ width:40, height:4, borderRadius:2, background:'var(--card-border)', margin:'0 auto 16px' }} />
        <h2 style={{ fontFamily:'Noto Serif KR', color:'var(--gold)', margin:'0 0 16px', fontSize:'1.1rem', textAlign:'center' }}>새 장소 추가</h2>

        <div style={{ marginBottom:16 }}>
          <div style={{ width:'100%', height:140, borderRadius:14, background: preview ? 'transparent' : 'rgba(255,255,255,0.04)', border:'2px dashed rgba(212,175,55,0.3)', overflow:'hidden', display:'flex', alignItems:'center', justifyContent:'center', marginBottom:8 }}>
            {preview
          ...
          <div style={{ display:'flex', gap:8 }}>
            <button onClick={() => cameraRef.current?.click()} style={{ flex:1, padding:'9px', background:'rgba(255,255,255,0.06)', border:'1px solid var(--card-border)', borderRadius:10, color:'var(--cream)', fontFamily:'Noto Sans KR', fontSize:'0.8rem', cursor:'pointer' }}>📷 카메라</button>
            <button onClick={() => fileRef.current?.click()} style={{ flex:1, padding:'9px', background:'rgba(255,255,255,0.06)', border:'1px solid var(--card-border)', borderRadius:10, color:'var(--cream)', fontFamily:'Noto Sans KR', fontSize:'0.8rem', cursor:'pointer' }}>🖼️ 사진첩</button>
          </div>
        ...
        <input value={form.nameKo} onChange={e => setForm(p => ({ ...p, nameKo: e.target.value }))} placeholder="* 업체명 (한국어) — 필수" style={{ ...inp, border: !form.nameKo ? '1px solid var(--crimson)' : '1px solid var(--card-border)' }} />
        ...
        <div style={{ display:'flex', gap:10, marginTop:8 }}>
          <button onClick={onClose} style={{ flex:1, padding:'13px', borderRadius:100, border:'none', background:'rgba(255,255,255,0.08)', color:'var(--text-secondary)', fontFamily:'Noto Sans KR', fontWeight:600, cursor:'pointer' }}>취소</button>
          <button onClick={handleSubmit} disabled={!form.nameKo.trim()} style={{ flex:2, padding:'13px', borderRadius:100, border:'none', background: form.nameKo.trim() ? 'linear-gradient(135deg, var(--crimson), #8B0000)' : 'rgba(255,255,255,0.1)', color:'white', fontFamily:'Noto Sans KR', fontWeight:700, cursor: form.nameKo.trim() ? 'pointer' : 'default', fontSize:'0.95rem' }}>저장</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ paddingBottom: 80 }}>
      ...
      <div style={{ padding: '16px 20px 12px' }}>
        <p style={{ color: 'var(--text-muted)', fontSize: '0.75rem', margin: '0 0 4px' }}>이우 생활권</p>
        <h1 style={{ fontFamily: 'Noto Serif KR', color: 'var(--cream)', fontSize: '1.5rem', margin: '0 0 4px' }}>⭐ 즐겨찾기</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, padding: '0 16px 12px' }}>
        {placeCategories.map(cat => (
          <button key={cat.id} onClick={() => handleCatChange(cat.id)}
            style={{ padding: '10px 8px', borderRadius: 12, border: 'none', background: selectedCat === cat.id ? 'var(--crimson)' : 'rgba(255,255,255,0.06)', color: selectedCat === cat.id ? 'white' : 'var(--text-secondary)', fontSize: '0.82rem', fontWeight: selectedCat === cat.id ? 700 : 400, cursor: 'pointer', fontFamily: 'Noto Sans KR', transition: 'all 0.2s', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
        ...
        <div style={{ display: 'flex', gap: 6, padding: '0 16px 12px', overflowX: 'auto', scrollbarWidth: 'none' }}>
          {subCategories[selectedCat].map(sub => (
            <button key={sub} onClick={() => setSelectedSub(sub)}
              style={{ flexShrink: 0, padding: '5px 14px', borderRadius: 100, border: selectedSub === sub ? '1px solid var(--gold)' : '1px solid rgba(212,175,55,0.2)', background: selectedSub === sub ? 'rgba(212,175,55,0.15)' : 'transparent', color: selectedSub === sub ? 'var(--gold)' : 'var(--text-muted)', fontSize: '0.75rem', fontWeight: selectedSub === sub ? 700 : 400, cursor: 'pointer', fontFamily: 'Noto Sans KR', whiteSpace: 'nowrap' }}>
              {sub}
            </button>
          ))}
        </div>
      ...
      <div style={{ padding:'0 16px 12px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        ...
        <button onClick={() => setShowAddModal(true)}
          style={{ background:'linear-gradient(135deg, var(--crimson), #8B0000)', color:'white', border:'none', borderRadius:100, padding:'8px 18px', fontSize:'0.82rem', fontWeight:700, cursor:'pointer', fontFamily:'Noto Sans KR', boxShadow:'0 3px 0 rgba(139,0,0,0.3)' }}>
          + 장소 추가
        </button>
      </div>
      ...
      <div style={{ padding: '0 10px' }}>
        {filtered.length === 0 ? (
          ...
          filtered.map(place => {
            ...
            return (
              <div key={place.id} className="card fade" style={{ marginBottom: 10, padding: 0, overflow: 'hidden', position: 'relative' }}>
                {place.isCustom && (
                  <button onClick={() => handleDeleteCustomPlace(place.id)}
                    style={{ position:'absolute', top:8, right:8, background:'rgba(196,30,58,0.8)', border:'none', borderRadius:'50%', width:28, height:28, color:'white', cursor:'pointer', fontSize:'0.8rem', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1 }}>
                    🗑
                  </button>
                )}
                ...
                <div style={{ width: '100%', height: 160, background: place.images?.[0] ? 'transparent' : 'linear-gradient(135deg, rgba(196,30,58,0.12), rgba(212,175,55,0.08))', overflow: 'hidden', borderRadius: '14px 14px 0 0', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, position: 'relative' }}>
                  {place.images?.[0] ? (
                    <img src={place.images[0]} alt={place.nameKo} style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                ...
                <div style={{ padding: '12px 14px' }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:2 }}>
                    <p style={{ margin:0, fontWeight:700, fontSize:'0.95rem', color:'var(--text-primary)', flex:1 }}>{place.nameKo}</p>
                  ...
                  <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
                    {place.distance && <span style={{ fontSize: '0.68rem', padding: '2px 8px', background: 'rgba(212,175,55,0.1)', borderRadius: 100, color: 'var(--text-muted)' }}>📏 {place.distance}</span>}
                    {place.driveTime && <span style={{ fontSize: '0.68rem', padding: '2px 8px', background: 'rgba(196,30,58,0.08)', borderRadius: 100, color: 'var(--text-muted)' }}>🚗 {place.driveTime}</span>}
                  </div>

                  {place.description && <p style={{ margin: '0 0 8px', fontSize: '0.78rem', color: 'var(--text-secondary)', lineHeight: 1.5 }}>{place.description}</p>}
                  <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                    {place.tags?.map(tag => (<span key={tag} className="tag">#{tag}</span>))}
                  </div>

                  <button onClick={() => openAmapSearch(place.nameZh || place.nameKo)}
                    style={{ width: '100%', marginTop: 12, padding: '11px', background: 'linear-gradient(135deg, #C41E3A, #8B0000)', color: 'white', border: 'none', borderRadius: 12, fontSize: '0.85rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'Noto Sans KR', boxShadow: '0 3px 0 rgba(139,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                    🗺️ 고덕지도에서 검색
                  </button>
                </div>
              </div>
      ...
      {showAddModal && <AddPlaceModal onClose={() => setShowAddModal(false)} onSave={handleSavePlace} />}
    </div>
  );
}
